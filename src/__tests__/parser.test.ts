...
describe("TaskParser.extractDate", () => {
	...
	it("extracts date from full task text", () => {
		expect(
			TaskParser.extractDate("- [ ] Team Meeting 📆 2025-01-15 🔔"),
		).toBe("2025-01-15");
	});
...
describe("TaskParser.extractDateFromFilename", () => {
	...
	it("extracts from filename with title", () => {
		expect(
			TaskParser.extractDateFromFilename("2026-01-07 My Notes.md"),
		).toBe("2026-01-07");
	});

	it("extracts from path with directories", () => {
		expect(
			TaskParser.extractDateFromFilename("Daily/2026-01-07.md"),
		).toBe("2026-01-07");
		expect(
			TaskParser.extractDateFromFilename(
				"Notes/Daily/2026-01-07 Meeting.md",
			),
		).toBe("2026-01-07");
	});

	it("handles files without .md extension", () => {
		expect(TaskParser.extractDateFromFilename("2026-01-07")).toBe(
			"2026-01-07",
		);
	});
	...
	it("returns null for no date in filename", () => {
		...
		expect(
			TaskParser.extractDateFromFilename("Meeting Notes.md"),
		).toBeNull();
	});
...
describe("TaskParser.extractTitle", () => {
	...
	it("handles various checkbox formats", () => {
		expect(
			TaskParser.extractTitle("- [x] Done Task 📆 2025-01-15"),
		).toBe("Done Task");
		expect(
			TaskParser.extractTitle("* [ ] Bullet Task 📆 2025-01-15"),
		).toBe("Bullet Task");
		expect(
			TaskParser.extractTitle("- [-] Progress Task 📆 2025-01-15"),
		).toBe("Progress Task");
	});
	...
	it("includes everything before date marker", () => {
		expect(
			TaskParser.extractTitle(
				"- [ ] Meeting with @person #tag 📆 2025-01-15",
			),
...
describe("TaskParser.extractBlockId", () => {
	...
	it("handles block IDs with hyphens", () => {
		expect(TaskParser.extractBlockId("task ^my-block-id")).toBe(
			"my-block-id",
		);
	});
